...
import { FooterDiv, MessageForm, SendButton, LogoImage, TitleDiv, LinkItem, CopyRight, Row, BackOverlay } from '../Styles/Footer.styles';
import { RopeLine, WaveDiv, WaveLine } from 'src/components/Landing/Styles/MintNow.styles';
...
const Footer = () => {
    ...
    return (
        <FooterDiv>
            ...
            <Row>
                <BackOverlay />
                <Grid container style={{marginTop : '50px', position : 'relative', zIndex : 10}} spacing={3}>
                    <Grid item xs={4}>
                        <LogoImage>
                            <img src={Logo} />
                        </LogoImage>
                    ...
                    <Grid item xs={4}>
                        ...
                        <Grid container sx={{marginTop : '30px'}}>
                            <Grid item xs={6}>
                                {
                                    linkList.slice(0, 4).map((item, index) => (
                                        <LinkItem key={index}>
                                            <ArrowForwardIosIcon /> {item.label}
                                        </LinkItem>
                                    ))
                                }
                            </Grid>
                            <Grid item xs={6}>
                                {
                                    linkList.slice(4, 6).map((item, index) => (
                                        <LinkItem key={index}>
                                            <ArrowForwardIosIcon /> {item.label}
                                        </LinkItem>
                                    ))
            ...
            <CopyRight>
                ...
                <p>© Copyright 2022 – Ship Life NFT Collection 888</p>
            </CopyRight>
        </FooterDiv>
    )   
}
